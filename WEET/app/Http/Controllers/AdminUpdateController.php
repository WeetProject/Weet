...
class AdminUpdateController extends Controller
{
    public function adminManagementUpdate(Request $request) 
    {
        try {
            ...
            Log::debug("### Admin권한 변경 요청 : " . $confirmAdminInfo->admin_flg . " ###");

            if ($confirmAdminInfo->admin_flg === 1) {
                
            } elseif ($confirmAdminInfo->admin_flg === 2) {
                Log::debug("### Admin권한 요청 플래그 : Admin ###");
            }


            if ($confirmAdminInfo->admin_flg === 1) {
                Log::debug("### Admin권한 변경 요청 : Sub(1) ###");
                $confirmAdminInfo->admin_flg = 2;
                Log::debug("### Admin권한 변경 처리 : Root(2) ###");
            } else if($confirmAdminInfo->admin_flg === 2) {
                Log::debug("### Admin권한 변경 요청 : Root(2) ###");
                $confirmAdminInfo->admin_flg = 1;
                Log::debug("### Admin권한 변경 처리 : Sub(1) ###");
            }
